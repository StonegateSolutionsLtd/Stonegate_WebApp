...
export default function ServiceQuoteEditor({ order }: { order: Order }) {
  ...
  // Base = selected load price (editable), hours always = 1
  const parsedLoadPrice = parseFloat(customPrice) || 0
  const total = Math.round((parsedLoadPrice + parsedFees) * 100) / 100
  ...
  async function handleSaveAndPrint() {
    ...
    if (tab === 'standard') {
      ...
      const res = await fetch(`/api/admin/service-orders/${order.id}/quote`, {
        ...
        body: JSON.stringify({ quote_type: 'standard', hourly_rate: p, estimated_hours: 1, additional_fees: f }),
      })
      if (res.ok) {
        router.refresh()
        ...
        <div style={{ background: 'white', borderRadius: '12px', padding: '24px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          ...
          <div style={{ display: 'flex', gap: '6px', marginBottom: '20px', background: '#FAF7F2', borderRadius: '10px', padding: '4px' }}>
            {(['standard', 'detailed'] as const).map(t => (
              ...
              >
                {t === 'standard' ? 'Custom / Volume' : 'Detailed Quote'}
              </button>
            ))}
          </div>
          ...
          {tab === 'detailed' && <DetailedQuoteForm {...detailedFormProps} />}
            ...
            {tab === 'standard' ? (
              ...
              <>
                ...
                {detailedResult.lineItems.map(li => (
                  <SummaryRow key={li.label} label={li.label} value={`$${li.amount.toFixed(2)}`} />
                ))}
                <div style={{ borderTop: '1px solid #E8E0D5', marginTop: '8px', paddingTop: '8px', display: 'flex', justifyContent: 'space-between', fontSize: '15px', fontWeight: 800, color: '#254220' }}>
